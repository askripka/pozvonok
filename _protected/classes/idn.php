...
class IDN {
    ...
    private static function decodeDigit($cp) {
        $cp = strtolower($cp);
        if($cp >= 'a' && $cp <= 'z')
            return ord($cp) - ord('a'); elseif($cp >= '0' && $cp <= '9')
    ...
    private static function utf8($cp) {
        ...
        if($cp < 2097152)
            return chr(240 + ($cp >> 18)).chr(128 + (($cp >> 12) & 63)).chr(128 + (($cp >> 6) & 63)).chr(128 + ($cp & 63));
    }
    ...
    private static function decodePart($input) {
        ...
        while(!empty($input)) {
            ...
            $bias = IDN::punyAdapt($i - $oldi, count($output) + 1, $oldi == 0);
    ...
    public static function decodeIDN($name) {
        return implode(".", array_map("IDN::decodePart", explode(".", $name)));
    }
