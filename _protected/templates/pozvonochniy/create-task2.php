...
$tariff = $api->get_tariff($api->user()['tariff']);
$tariff_mml = $tariff['max_message_length'] ? $tariff['max_message_length'] : 1000;
$tariff_mcd = $tariff['max_call_duration'] ? $tariff['max_call_duration'] : 60;
$emotions = array(
    "mixed" => "Переменная",
...
<script>
    window.onload = function () {
        app.recorder.timeLimit = <?=$tariff_mcd?>;


        <?if($task):?>
        app.taskConstructor.task = <?=json_encode($task)?>;
        app.taskConstructor.task.scripts = <?=json_encode($api->get_task_scripts($task['id']))?>;
        <?endif;?>
        ...
        <div class="form-group">
            <div class="clearfix">
                <div class="col-sm-9">
                    ...
                    <div class="form-group">
                        <div class="row">
                            ...
                            <div class="col-sm-6">
                                <input name="date_start" type="text" id="taskFormDatetime" class="form-control" data-plugin="datetimepicker" data-options="{ format:'YYYY-MM-DD HH:mm', defaultDate: '<?= date("Y-m-d H:00", strtotime(date("Y-m-d H:i:s")." +1 hour")); ?>', minDate: '<?= date("Y-m-d H:00", strtotime(date("Y-m-d H:i:s")." +1 hour")); ?>', locale: 'ru' }" value="" required>
                            </div>
                            <div class="col-sm-4">
                ...
                <div class="col-sm-3 text-right">
                    <b>ПРИБЛИЗИТЕЛЬНАЯ</b><br>
                    <span>Длительность звонка: <b>100</b>мин</span><br>
                    <span>Стоимость звонка: <b>120</b><?=API::CURRENCY?></span><br>
                    <span>Стоимость СМС: <b>10</b><?=API::CURRENCY?></span><br>
                    <span>Стоимость соединения с оператором: <b>13</b><?=API::CURRENCY?></span><br>
                    ---<br>
                    <span>Всего: <b>1300</b><?=API::CURRENCY?></span>
                </div>
            </div>
        </div>
        <div class="form-group">
                            ...
                            <div class="col-sm-3">
                                ...
                                <div class="task-script-phone">
                                    ...
                                    <div class="task-script-voice-actions">
                                        <? for ($i = 1; $i <= 5; $i++): ?>
                                            <button class="btn btn-default" type="button" data-action-type="voice_actions" data-action-id="<?= ($i) ?>" onclick="app.taskConstructor.modalOpen(this);">Слово <?= ($i) ?></button>
                                        <? endfor; ?>
                                    </div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p class="text-muted">
                    <b>Запись звука работает в браузерах Chrome и Firefox. Используйте эти современыне браузеры.</b>
                    <br>
                    Максимальный размер файла: 5Мб
                    <br>
                    Максимально допустимая длина аудиозаписи по вашему тарифу: <?= $tariff_mcd ?> секунд.
                </p>
                ...
                <div class="form-group row">
                    <div class="col-sm-6">
                        <label class="btn btn-primary btn-block recupload">
                            ...
                            <input type="file" name="mp3" class="audioinput" style="display: none;" accept=".mp3" onchange="app.taskConstructor.recBuffers(this);"/>
                        </label>
                    </div>
                    <div class="col-sm-6">
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>
                    <small>
                        Максимально допустимая длина текста по вашему тарифу: <?= intval($tariff_mml / 3) ?> символов.
                    </small>
                </p>
                <div class="form-group">
                    <textarea class="form-control" name="message" placeholder="Введите текст основного сообщения" maxlength="<?= intval($tariff_mml / 3) ?>" data-plugin="maxlength" data-options="{  threshold: 10, warningClass: 'label label-warning', limitReachedClass: 'label label-danger', placement: 'bottom', message: '%charsTyped% / %charsTotal%' }"></textarea>
                </div>
                <div class="form-group">
...
<div class="modal fade" id="modalVoiceActions" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="form-group">
                    <input type="text" name="keyword" value="" placeholder="Введите ключевое слово для распознавания" class="form-control" maxlength="70" data-plugin="maxlength" data-options="{ threshold: 10, warningClass: 'label label-warning', limitReachedClass: 'label label-danger', placement: 'bottom', message: '%charsTyped% / %charsTotal%' }"/>
                </div>
                <br/>
                <div class="row form-group">
                    ...
                    <div class="col-sm-6">
                        ...
                        <div class="form-group">
                            ...
                            <div class="radio">
                                <input id="radio006" type="radio" name="after_action" value="next" checked/>
